...
const MiniDonut = ({ gradeDist, total }) => {
  ...
  return (
    <svg width="88" height="88">
      ...
      <text x={CX} y={CY - 4} textAnchor="middle" dominantBaseline="middle" style={{ fontSize: '11px', fontWeight: 800, fill: '#18181b', fontFamily: 'Outfit, sans-serif' }}>
        {total}
      </text>
      <text x={CX} y={CY + 10} textAnchor="middle" dominantBaseline="middle" style={{ fontSize: '7.5px', fill: '#71717a', fontWeight: 600, fontFamily: 'Inter, sans-serif' }}>
        RECORDS
      </text>
    </svg>
  );
...
const TeacherAnalytics = ({ progressList = [], allScores = [], allLearners = [], settings = null, currentTerm = '', currentAcademicYear = '' }) => {
  const analyticsData = useMemo(() => {
    return progressList.map(item => {
      ...
      const avgColor = classAvg === null ? '#71717a' : classAvg >= 70 ? '#10B981' : classAvg >= 50 ? '#F59E0B' : '#EF4444';
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '1.25rem', display: 'flex', alignItems: 'center', gap: '12px' }}>
        <div style={{ width: '38px', height: '38px', borderRadius: '12px', background: '#09090b', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <i className="fas fa-chart-pie" style={{ color: 'white', fontSize: '0.9rem' }} />
        </div>
        <div>
          <h2 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 800, color: '#09090b' }}>
      ...
      {!hasAnyScores ? (
        <div style={{ padding: '1.5rem', background: '#FAFAFA', borderRadius: '12px', border: '1px dashed #E4E4E7', textAlign: 'center', color: '#71717a' }}>
          <i className="fas fa-pencil-alt" style={{ fontSize: '1.5rem', color: '#2563eb', opacity: 0.4, marginBottom: '0.5rem', display: 'block' }} />
          ...
          {analyticsData.map(item => (
            ...
            >
              ...
              <div style={{ marginBottom: '0.75rem' }}>
                <div style={{ fontWeight: 800, fontSize: '0.9rem', color: '#09090b' }}>{item.className}</div>
                <div style={{ fontSize: '0.72rem', color: '#71717a', fontWeight: 500 }}>{item.subjectName}</div>
              </div>

              {item.gradedCount === 0 ? (
                <div style={{ textAlign: 'center', padding: '1rem', color: '#71717a', fontSize: '0.78rem' }}>
                  <i className="fas fa-clock" style={{ display: 'block', marginBottom: '4px', color: '#2563eb', opacity: 0.5 }} />
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {item.classAvg !== null && (
                      <div style={{ marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <span style={{ fontSize: '0.65rem', fontWeight: 700, color: '#71717a', textTransform: 'uppercase' }}>Avg:</span>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      {item.gradeDist.slice(0, 5).map(gd => (
                        <div key={gd.grade} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          <span style={{ fontSize: '0.7rem', fontWeight: 700, color: '#18181b', minWidth: '18px' }}>{gd.grade}</span>
                          ...
                          <span style={{ fontSize: '0.65rem', color: '#71717a', fontWeight: 600, minWidth: '16px', textAlign: 'right' }}>{gd.count}</span>
                        </div>
                      ))}
                    </div>
                  </div>
